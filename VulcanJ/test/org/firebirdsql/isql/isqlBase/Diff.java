...
class fileInfo {
	...
	// wbo fix deprecated warning. was...
 ...
 * 
 * This program assumes that "oldfile" and "newfile" are text files. The program
 * writes to stdout a description of the changes which would transform "oldfile"
 * into "newfile".
 * 
 * The printout is in the form of commands, each followed by a block of text.
 * The text is delimited by the commands, which are:
 * 
 ...
 * 
 * The line numbers all refer to the lines of the oldfile, as they are numbered
 * before any commands are applied. The text lines are printed as-is, without
 * indentation or prefixing. The commands are printed in upper case, with a
 * prefix of ">>>>", so that they will stand out. Other schemes may be
 * preferred. Files which contain more than MAXLINECOUNT lines cannot be
 * processed. This can be fixed by changing "symbol" to a Vector. The algorithm
 * is taken from Communications of the ACM, Apr78 (21, 4, 264-), "A Technique
 ...
 * "right". However, it is often excellent. This program is intended to be
 * pedagogic. Specifically, this program was the basis of the Literate
 * Programming column which appeared in the Communications of the ACM (CACM), in
 * the June 1989 issue (32, 6, 740-755). By "pedagogic", I do not mean that the
 * program is gracefully worded, or that it showcases language features or its
 * algorithm. I also do not mean that it is highly accessible to beginners, or
 ...
 * 
 * Change Log ---------- 1Jan97 Ian F. Darwin: first working rewrite in Java,
...
public class Diff {
	String[] stringsToEat = {"C:\\\\WINNT\\\\SYSTEM32\\\\TEST.G00",
			...
			"Forced Writes are OFF", "Forced Writes are ON", "SQL> "};

	int match = 1; // 0 for match, 1 for fail
	...
	public int doDiff(String oldFile, String newFile) {
	...
	/**
	 * inputscan Reads the file specified by pinfo.file. --------- Places the
	 * lines of that file in the symbol table. Sets pinfo.maxLine to the number
	 * of lines found.
	 */
...
class node { /* the tree is made up of these nodes */
	 ...
	 * exactly once with inoldfile true, and exactly once with inoldfile false.
	 */
